  ...
  return (
    <div className="space-y-12 py-8 max-w-5xl mx-auto px-4 md:px-8">
      ...
      <div className="border-b border-border pb-6">
        <span className="text-xs font-semibold tracking-widest text-muted-foreground uppercase">Reporting</span>
        <h1 className="text-3xl font-normal tracking-tight text-foreground mt-1">Financial Analytics</h1>
        <p className="text-sm text-muted-foreground mt-1">Analyze profitability, top clients, and project tax allocations.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="rounded-lg border border-border bg-background shadow-none">
          <CardContent className="p-6">
            ...
            <div className="mt-4 space-y-1">
              <h3 className="text-2xl font-normal tracking-tight text-foreground">{revenue.toLocaleString()} DA</h3>
              <p className="text-xs text-muted-foreground">Total invoices and revenue logged</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-lg border border-border bg-background shadow-none">
          <CardContent className="p-6">
            ...
            <div className="mt-4 space-y-1">
              <h3 className="text-2xl font-normal tracking-tight text-foreground">{expenses.toLocaleString()} DA</h3>
              <p className="text-xs text-muted-foreground">All business related costs</p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <Card className="rounded-lg border border-border shadow-none bg-background overflow-hidden">
          <CardHeader className="bg-muted border-b border-border p-4">
            <CardTitle className="text-sm font-medium text-foreground uppercase tracking-wider flex items-center gap-2">
              <Award className="h-4 w-4 text-destructive" /> Service Category Rankings
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            {serviceRanking.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-8">No service data available.</p>
            ) : (
                ...
                return (
                  <div key={idx} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">{s.value.toLocaleString()} DA</span>
                    </div>
                    {/* Visual Bar */}
        ...
        <Card className="rounded-lg border border-border shadow-none bg-background overflow-hidden">
          <CardHeader className="bg-muted border-b border-border p-4">
            <CardTitle className="text-sm font-medium text-foreground uppercase tracking-wider flex items-center gap-2">
              <Award className="h-4 w-4 text-destructive" /> Client Rankings
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            {clientRanking.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-8">No client billing logged.</p>
            ) : (
                ...
                return (
                  <div key={idx} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">{c.revenue.toLocaleString()} DA</span>
                    </div>
                    {/* Visual Bar */}
                    <div className="w-full h-2.5 bg-muted rounded-full overflow-hidden">
                      <div
      ...
      <div className="bg-accent/50 text-foreground p-8 rounded-lg space-y-4">
