  ...
  return (
    <div className="space-y-8 py-8 max-w-5xl mx-auto px-4 md:px-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-hairline pb-6">
        <div>
          <span className="text-xs font-semibold tracking-widest text-muted-custom uppercase">Deadlines</span>
          <h1 className="text-3xl font-normal tracking-tight text-ink mt-1">Calendar & Schedule</h1>
          <p className="text-sm text-body-text mt-1">Track regulatory tax filings and social security payments.</p>
        </div>
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="sm:max-w-md bg-canvas">
            <DialogHeader>
              <DialogTitle className="text-xl font-normal tracking-tight text-ink">Add Reminder</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4 pt-2">
              <div className="space-y-1">
                <label className="text-xs uppercase font-medium text-muted-custom">Reminder Title</label>
                <Input
                  required
                  value={title}
              ...
              <div className="space-y-1">
                <label className="text-xs uppercase font-medium text-muted-custom">Details</label>
                <textarea
                  value={description}
      ...
      <div className="bg-signature-cream text-ink p-6 rounded-lg grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-1">
          <h3 className="font-semibold text-base flex items-center gap-2">
            <AlertTriangle className="h-4 w-4 text-signature-coral" /> Algerian Auto-Entrepreneur Calendar Defaults
          </h3>
        ...
        <div className="flex items-center justify-end">
          <div className="text-right text-xs border-l border-hairline pl-4">
            <span className="font-semibold text-ink block">CASNOS Due:</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
        ...
        <div className="space-y-4">
          <h2 className="text-sm font-semibold tracking-wider text-muted-custom uppercase">Active Deadlines</h2>
          <div className="border border-hairline rounded-lg divide-y divide-hairline bg-canvas overflow-hidden">
            {isLoading ? (
              ...
              <div className="p-6 text-center text-sm text-muted-custom">No active deadlines. Great job!</div>
            ) : (
              reminders
                .filter((r: any) => !r.isCompleted)
                .map((r: any) => (
                  <div key={r.id} className="p-4 flex items-start gap-3 bg-canvas">
                    <button
                      onClick={() => toggleMutation.mutate({ id: r.id, isCompleted: true })}
                      className="text-muted-custom hover:text-success mt-0.5"
                    >
                      <Circle className="h-5 w-5" />
                    </button>
                    <div className="flex-1">
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="text-sm font-semibold text-ink leading-tight">{r.title}</h4>
                        <span className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded bg-surface-soft text-muted-custom shrink-0 border border-hairline">
                          {r.type}
                        </span>
                      </div>
                      ...
                      {r.description && <p className="text-xs text-body-text mt-2">{r.description}</p>}
                    </div>
                  </div>
                ))
            )}
        ...
        {/* Settled Deadlines */}
        <div className="space-y-4">
          <h2 className="text-sm font-semibold tracking-wider text-muted-custom uppercase">Settled Deadlines</h2>
          <div className="border border-hairline rounded-lg divide-y divide-hairline bg-canvas overflow-hidden opacity-75">
            {reminders.filter((r: any) => r.isCompleted).length === 0 ? (
              <div className="p-6 text-center text-sm text-muted-custom">No completed items.</div>
            ) : (
              reminders
                .filter((r: any) => r.isCompleted)
                .map((r: any) => (
                  <div key={r.id} className="p-4 flex items-start gap-3 bg-surface-soft">
                    <button
                    ...
                    <div className="flex-1 line-through">
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="text-sm font-medium text-muted-custom leading-tight">{r.title}</h4>
                        <span className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded bg-muted text-muted-custom shrink-0">
                          {r.type}
                        </span>
                      </div>
                      <p className="text-xs text-muted-custom mt-1">
